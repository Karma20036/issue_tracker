...
// Ensure the user being revoked belongs to the same company
$check_sql = "SELECT role, company_name FROM users WHERE id='$id'";
$check_result = $conn->query($check_sql);
if ($check_result->num_rows > 0) {
    $row = $check_result->fetch_assoc();
    if ($row['company_name'] === $company_name && $row['role'] !== 'Organization Admin') {
        ...
        echo "You cannot revoke access for this user.";
    }
} else {
    echo "User not found.";
}
